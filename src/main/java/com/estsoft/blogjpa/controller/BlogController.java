...
public class BlogController {
    ...
    public ResponseEntity<List<ArticleResponse>> showArticle() {
        ...
        List<ArticleResponse> responseList = articleList.stream().map(Article::toResponse).toList();
    ...
    public ResponseEntity<ArticleResponse> showOneArticle(@PathVariable Long id) {
        try {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ArticleResponse());
        }
    }
    ...
    public ResponseEntity<Article> updateOneArticle(@PathVariable Long id, @RequestBody UpdateArticleRequest request)
    {
